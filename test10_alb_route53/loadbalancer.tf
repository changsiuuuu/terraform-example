...
# 2. ALB 본체 (Load balancer L7) (Layer 7 에서 동작)
resource "aws_lb" "web_alb" {
  name               = "lecture-alb"
  internal           = false                          # 외부 노출용
  load_balancer_type = "application"                  # 로드밸런서종류
  security_groups    = [aws_security_group.alb_sg.id] # 위에서 정의한 보안그룹 적용
  # 고가용성을 위해 최소 2개의 퍼블릭 서브넷 제공
...
# 3. ALB 가 받은 요청을 최종적으로 전달할 대상 
resource "aws_lb_target_group" "web_tg" {
  name = "lecture-tg"
  # 대상 ec2 에서 돌아가는 web 서버의 포트 번호 변경가능
  port     = 80
  protocol = "HTTP"
  vpc_id   = aws_vpc.main.id
...
# 모든 ALB로부터 요청을 전달받을 모든 EC2를 대상 그룹 target 에 등록한다. 
