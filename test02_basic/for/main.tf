...
output "debug07_multiline" {
    # value = <<-EOF 를 쓴다. 문자열 리다이렉션을 쓸 수 있다..
    # <<- 기호를 쓰면 좌측의 공백이 알아서 제거가 된다.
    # EOF는 마음대로 정할 수 있는 글자 끝날때만 동일한 문자열로 끝내면된다..
  value = <<-EOF
      #!/bin/bash
      dnf update -y
      dnf install -y nginx
