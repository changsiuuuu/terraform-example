...
terraform {
  ...
  backend "s3" {
    bucket = "tfstate-bucket-b2621cea" # 미리 생성한 s3 버킷의 이름
    key = " lecture/terraform.tfstate " # /lecture/하위에 만들어 지도록
    region = "ap-northeast-2"
    dynamodb_table = "terraform-lock" # 미리 준비된 dynamodb 테이블의 이름 명시.. lock 상태가 자동으로 관리
...
resource "aws_route_table" "public_rt" {
  # 어떤 vpc의 소속인지 설정
  vpc_id = aws_vpc.main.id
  # 라우팅 규칙 (0.0.0.0/0) 으로 가는 트래픽은 인터넷게이트웨이igw로
  route {
...
# 개인 키를 가져오기
# local_file resource를 이용하면 파일을 생성 할 수 있다.
...
resource "aws_security_group" "ssh_sg" {
  # 보안 그룹의 이름은 겹치지 않고 유일하게 식별되는 이름을 지어야 한다.
  name   = "allow-ssh"
  vpc_id = aws_vpc.main.id
  # 밖에서 안으로 들어오는 규칙은 ingress
  ingress {
    from_port   = 22            # 시작 port
    to_port     = 22            # 끝 port
    protocol    = "tcp"         # protocol
    cidr_blocks = ["0.0.0.0/0"] # 외부에서 들어오는 모든 taffic (실무에서는 나의 ip만)
  }
  ingress {
...
resource "aws_instance" "my_ec2" {
  ...
  subnet_id              = aws_subnet.public_subnet.id    # 위에서 미리 준비한 퍼블릭 서브넷의 아이디
  vpc_security_group_ids = [aws_security_group.ssh_sg.id] # 보안 그룹 (여러개 등록가능)
  key_name               = aws_key_pair.kp.key_name       # 위에서 준비한 key pair 의 이름
  # 다른 ec2에서 mgmt 쪽으로 보내는 traffic을 전달 할 수 있도록 한다
  source_dest_check = false # routing 기능을 수행한다.
  tags = {
    Name = "my-ec2"
  }

  # ec2 가 provisioning 될때 실행할 script를 지정할 수 있다.
  user_data = <<-EOF
    #!/bin/bash
    # host 이름 변경
...
# public ip를 이용해서 inventory.yml 파일만들기
resource "local_file" "ansible_inventory" {
    # 파일 경로와 파일명
    filename = "${path.module}/inventory.yml"
    # 파일 내용을 map 객체를 이용해서 구성하기
    content = yamlencode({
        all = {
            hosts = {
...
# 인프라 생성 후 ansible-playbook 을 실행 가능한 시간 만큼 대기한다
resource "terraform_data" "wait_for_instance" {
    ...
    # ec2 인스턴스의 id 가 변경된다면 다시 실행하도록 조건을 준다.
    # ec2가 새롭게 만들어지면 이 블럭이 다시 실행되고 결과적으로 sleep 30 이 다시 실행된다.
...
resource "terraform_data" "ansible_run" {
    ...
    # 아래 작업이 성공했던 기록이있으면 다시 또 실행되지 않는다..
    triggers_replace = {
        instance_id = aws_instance.my_ec2.id
        always_run = "${timestamp()}" # 실행할때마다 시간이 달라지므로 무조건 플레이북이 실행된다.
    }
    provisioner "local-exec" {
        # db 설치하고 파일 copy 하고 권한변경이 잦고 하면 에러가 날 수 있기 때문에 안정적으로 실행 하기 위해
        # ANSIBLE_SSH_PIPELINING=1 라는 임시 환경 변수를 추가한다.connection 
        # 해당 환경변수는 ansible playbook 이 실행될 때에 한해서 ansible이 사용하는 일시적인 환경 변수이다.
        command = "ANSIBLE_SSH_PIPELINING=1 ansible-playbook site.yml --extra-vars 'host_name=${var.host_name}'"
    }
}

# host 명으로사용할 변수를 선언하고 기본값 전달
variable "host_name" {
    type = string
    default = "db-server"
}