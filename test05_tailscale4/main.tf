...
resource "aws_route_table" "public_rt" {
    vpc_id = aws_vpc.main.id
    # vpc 대역이 아닌 다른 모든 ip 에 대한 요청은 internet gateway로
    route {
...
# public subnet 을 public_rt 와 연결
resource  "aws_route_table_association" "public_a" {
    # public subnet을 
    subnet_id      = aws_subnet.public_subnet.id 
    # 여기 정의된 routing table 로 연결하겠다는 의미
...
resource "aws_instance" "nat_ec2" {
    ...
    subnet_id              = aws_subnet.public_subnet.id
    # 외부로 나가야하기 때문에 public ipv4 주소 필요
    associate_public_ip_address = true
    vpc_security_group_ids = [aws_security_group.nat_sg.id]
    key_name               = aws_key_pair.kp.key_name
    # 다른 ec2에서 받은 트래픽을  외부로 전달해야해서 
    source_dest_check      = false    # NAT 인스턴스 필수!

    # 부팅 시 NAT 설정 (masquerade 및 routing) 을 해줘야한다.. 
    user_data = <<-EOF
        #!/bin/bash
                set -eux
        # 1. IP 포워딩 활성화
...
# aws_route는 미리 만들어놓은 aws_route_table 에 라우팅 정보를 추가하는 용도로 사용할수잇다
...
resource "aws_instance" "my_ec2" {
    ...
    # 대장 ec2 가 만들어지는 시점에 tailscale 에 접속해서 vpn 연결을 활성화하는 script 작성
    # 조금 복잡하지만 다음 예제에선 이걸 tailscale terraform 의 기능을 이용해서 단순화
    user_data = <<-EOF
        #!/bin/bash
                # 로그 파일 생성 및 모든 출력 기록
...
data "tailscale_device" "my_ec2_device" {
  ...
  wait_for   = "60s" # 위의 대장 ec2 기기가 리스트에 뜰 때까지 테라폼이 기다려줍니다.
...
resource "terraform_data" "wait_for_instance"{
    ...
    provisioner "local-exec" {
        # user_data에서 Tailscale 세팅이 완벽히 끝날 때까지 넉넉하게 대기 (90초)
