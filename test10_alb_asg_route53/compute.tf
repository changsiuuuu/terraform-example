# test10_alb_asg_route53/compute.tf
...
resource "aws_launch_template" "lt" {
  ...
  # ami 선택 (직접만든 ami를 선택할수도있다.)
  image_id = data.aws_ami.latest_al2023.id
  # 인스턴스 타입
  instance_type = var.instance_type # 변수에 있는 내용 참조
  # 생성되는 ec2가 공통으로 사용할 보안그룹
  vpc_security_group_ids = [aws_security_group.asg_sg.id]
  # 사용할 키의 이름
  key_name = aws_key_pair.kp.key_name
  # 프로비저닝 후에 실행할 user_data (테스트용으로 nginx 설치 및 시작)
...
# 5. 위 설계도를 이용해서 실제 동작할 Auto Scailing Group 정의
resource "aws_autoscaling_group" "asg" {
  ...
  # ALB의 target group arn 을 여기 등록한다.
